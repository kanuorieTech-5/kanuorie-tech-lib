...
export default function Library() {
  ...
  const loadLibrary = async () => {
    try {
      ...
      await Promise.all([fetchResources(), fetchSavedCourses()]);
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchResources = async () => {
    try {
      ...
      */
      ...
      const combined = [...defaultResources, ...data];

      const formatted = combined.map((item, index) => ({
        ...item,

        resourceId: item._id || item.id || `${item.title}-${index}`,
      }));
      ...
      */
      ...
      setResources(
        defaultResources.map((item, index) => ({
          ...
          resourceId: item.id || `${item.title}-${index}`,
        })),
      );
    }
  };
  ...
  const fetchSavedCourses = async () => {
    ...
    */
      ...
      const ids = data
        .map((course) => course.resourceId || course._id || course.id)
        .filter(Boolean);
  ...
  const recommendedResources = useMemo(() => {
    ...
    return resources.filter((resource) => {
      ...
      return (
        savedCategories.includes(resourceCategory) &&
        !savedIds.includes(resource.resourceId)
      );
    });
  }, [resources, savedResources, savedIds]);

  const totalPages = Math.ceil(filteredResources.length / BOOKS_PER_PAGE);
  ...
  const handleSave = async (resource) => {
    ...
    */
      ...
      const savedCourse = response?.data?.course || response?.data || response;

      const savedResourceId = savedCourse?.resourceId || resource.resourceId;
      ...
      setSavedIds((previous) => {
        ...
        return [...previous, savedResourceId];
      });
  ...
  const hasResources = paginatedResources.length > 0;
      ...
      <section className="bg-slate-200 py-10">
        <div className="px-6">
          ...
          <p className="mt-4 max-w-2xl text-gray-600">
            Explore curated books, tutorials, and resources designed for modern
            developers.
          </p>
